...
if rank == 0:
    ...
    # latent heat
    a_macro += 2 * np.pi * growth_speed * u_macro * radius_fn * phi_macro * dx_macro

    f_macro = (inner(rho_macro * old_effective_density_fn * theta_old, phi_macro) / dt \
            + f_stopper * inner(f_macro_prod, phi_macro)) * dx_macro

    # latent heat
    f_macro += 2 * np.pi * growth_speed * theta_ref * radius_fn * phi_macro * dx_macro

    ## dummy cell to determine cells dofs that should be coupled with the
    ## Dirichlet condition
    dummy_cell = CellProblemParallel(micro_mesh, kappa_micro, f_micro_prod, 
...
## All other processecs handle the cell problems
    ...
    for _ in range(dofs_macro_local):
        micro_problem_list.append(
            CellProblemParallel(micro_mesh, kappa_micro, f_micro_prod, 
                                growth_speed, theta_ref, theta_start, dt, 
                                micro_radius, rho_micro))
...
print(rank, "waiting for macor- and cell-problem definition")
comm.barrier()
# %%
...
while t_n <= T_int[1] - dt/8.0:
    ...
    if rank == 0:
        ...
        for i in range(dofs_macro_local):
            ...
            data_for_macro_domain[i, 0] = \
                assemble(micro_problem_list[i].theta_old * 
                            micro_problem_list[i].dx_micro)

            ## Update cells
            micro_problem_list[i].update_cell(macro_solution_array[i])
